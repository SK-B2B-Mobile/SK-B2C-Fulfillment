[README (1).md](https://github.com/user-attachments/files/30194290/README.1.md)
...
변경 이력 관리 용도**입니다. GitHub의 내용을 바꿔도 Apps Script가 자동으로 바뀌지는
...
1. Apps Script 에디터에서 `Code.gs` 전체 코드를 복사
2. 이 폴더의 `Code.gs`에 붙여넣기 → GitHub에 커밋
3. `appsscript.json`도 프로젝트 설정(⚙️ 톱니바퀴 → 편집기 설정 → "appsscript.json 매니페스트 파일 표시") 켠 뒤 동일하게 복사
